...
{
    public partial class CtrlProductGroup : UserControl
    {        
        public CtrlProductGroup()
        {
            InitializeComponent();
        ...
        {
            DataForms.FrmEditProductGroup frm = new FitnessProject.DataForms.FrmEditProductGroup();
            frm.ShowDialog();
        ...
        private void tbtnEdit_Click(object sender, EventArgs e)
        {
            ...
            DataForms.FrmEditProductGroup frm = new FitnessProject.DataForms.FrmEditProductGroup(ind);
            frm.ShowDialog();
        ...
        {
            if (MessageBox.Show("Удалить группу продуктов?", Lib.StringData.ProjectName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                int[] i;
                int SelRow = -1;
                i = advBandedGridView1.GetSelectedRows();
                ...
                DBLayer.ProductGroup.Delete(ind);
