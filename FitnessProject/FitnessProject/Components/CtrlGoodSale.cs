...
{
    public partial class CtrlGoodSale : UserControl
    {
        #region Fields
        ...
        {
            DBLayer.Sales.Details det = DBLayer.Sales.GetDetails(this.Id);

            det.DeleteReason = args.Reason;
            det.Quantity = 0;
            det.DeleteDate = DateTime.Now;

            DBLayer.Sales.SetDelete(det);
            DBLayer.Sales.Update(det);
