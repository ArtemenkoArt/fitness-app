...
{
    public partial class CtrlClientType : UserControl
    {
        ...
        {
            DataForms.FrmEditClientType frm = new FitnessProject.DataForms.FrmEditClientType();
            frm.ShowDialog();
        ...
        private void tbtnEdit_Click(object sender, EventArgs e)
        {
            ...
            DataForms.FrmEditClientType frm = new FitnessProject.DataForms.FrmEditClientType(ind);
            frm.ShowDialog();
        ...
        {
            if (MessageBox.Show("Удалить тип клиентов?", Lib.StringData.ProjectName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                int[] i;
                int SelRow = -1;
                i = advBandedGridView1.GetSelectedRows();
                ...
                DBLayer.ClientTypes.Delete(ind);
