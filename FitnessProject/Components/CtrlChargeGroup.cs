...
{
    public partial class CtrlChargeGroup : UserControl
    {
        public CtrlChargeGroup()
        {
            InitializeComponent();
        ...
        {
            DataForms.FrmEditChargeGroup frm = new FitnessProject.DataForms.FrmEditChargeGroup();
            frm.ShowDialog();
        ...
        private void tbtnEdit_Click(object sender, EventArgs e)
        {
            ...
            DataForms.FrmEditChargeGroup frm = new FitnessProject.DataForms.FrmEditChargeGroup(ind);
            frm.ShowDialog();
        ...
        {
            if (MessageBox.Show("Удалить группу расходов?", Lib.StringData.ProjectName, MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                int[] i;
                int SelRow = -1;
                i = advBandedGridView1.GetSelectedRows();
                ...
                DBLayer.ChargeGroup.Delete(ind);
